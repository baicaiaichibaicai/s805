...
int gpio_amlogic_name_to_num(const char *name)
{
	...
	start=p;
	p=strcpy(p,name);
	if (!strncmp(name, "DIF_TTL_", 8) && !strncmp(p+9, "_P", 2)){
			num=*(p+8)-0x30;
			return DIF_TTL_0_P+num*2;
	}
	if (!strncmp(name, "DIF_TTL_", 8) && !strncmp(p+9, "_N", 2)){
		num=*(p+8)-0x30;
		return DIF_TTL_0_N+num*2;
	}
	if (!strncmp(name, "HDMI_TTL_", 9) && !strncmp(p+10, "_P", 2)){
		num=*(p+9)-0x30;
		return HDMI_TTL_0_P+num*2;
	}
	if (!strncmp(name, "HDMI_TTL_", 9) && !strncmp(p+10, "_N", 2)){
		num=*(p+9)-0x30;
...
static int m8b_set_highz(unsigned int pin)
{
	m8b_set_pullup(pin,0,0);
...
struct gpio_chip amlogic_gpio_chip={
	...
	.set_pullup=m8b_set_pullup,
	.set_highz=m8b_set_highz,
