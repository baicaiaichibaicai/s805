...
extern unsigned int _omni_mac_filter(struct sk_buff *skb,
									 unsigned int hook,
									 const struct net_device *in,
									 const struct net_device *out,
									 struct omni_xt_table *table);

#define OMNI_MAC_FILTER(skb, val)											\
	do {																	\
		const struct net *net = dev_net(skb->dev);							\
		const struct omni_xt_table_info *private =							\
								net->ipv4.omni_macfilter_filter->private;	\
																			\
		if (private->initial_entries == private->number) {					\
			 val = OMNI_ACCEPT;												\
			 break;															\
		 }																	\
																			\
		  val = _omni_mac_filter(skb, OMNI_MACF_IN, skb->dev, NULL,			\
								  net->ipv4.omni_macfilter_filter);			\
	} while (0)
