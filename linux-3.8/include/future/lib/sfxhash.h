...
#include "sfmemcap.h"
#include "sfhashfcn.h"
#include "future/ips/fips_util.h"
/*
*   ERROR DEFINES
*/
...
void *sfxhash_alloc_bootmem(int rows, int key_size, int data_size);
void sfxhash_preload_node(SFXHASH *h, int count, void *p_mem);
void sfxhash_preload_delete( SFXHASH * h );

static inline SFXHASH* sfxhash_new_node(int node, int nrows, int keysize, int datasize, unsigned long memcap,
		int anr_flag, int (*anrfunc)(void *key, void * data), int (*usrfunc)(void *key, void * data), int recycle_flag)
{
	return sfxhash_new(nrows, keysize, datasize, memcap, anr_flag, anrfunc, usrfunc, recycle_flag, SFGHASH_MEM_TLSF,
			    get_ips_mem_node(node));
}
#endif
