...
#ifdef CONFIG_OCTEON_FUTURE_BOARD	
		maxcpus = __builtin_popcount(cvmx_read_csr(CVMX_CIU_FUSE));
		num_processors = maxcpus;
    	if (strstr(arg,"numcores=") != NULL){
			num_processors = simple_strtoul(arg+9, NULL, 10);
		}
		serial = strstr (arg,"serial=");
		if (serial != NULL)
