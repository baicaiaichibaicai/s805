...
typedef struct iplip_entry_s {
	unsigned int		crc32;		/* crc32 of iplip_hdr */
	union {
		struct cs_iplip_hdr_s {
			struct ethhdr		ethh;
			...
			cs_l2tp_hdr_t		l2tph;
			cs_ppp_hdr_t		ppph;
		} __attribute__((packed)) iplip_hdr;
		unsigned char	      iplip_octet[sizeof(struct cs_iplip_hdr_s)];
	};
	unsigned char		valid;	/*0: invalid 1: valid*/
	unsigned char		dir;	/* 0: unknown, 1: upstream, 2: downstream */
} __attribute__((packed)) cs_iplip_entry_t;
...
typedef struct cs_iplip_ipc_msg_set_s {
	unsigned char		idx;
	unsigned char		resvd[3];
	cs_iplip_entry_t	iplip_entry;
} __attribute__ ((__packed__)) cs_iplip_ipc_msg_set_t;
...
typedef struct cs_tunnel_entry_s {
	...
	/* 0: PE --> WAN
	   1: WAN --> PE */
	unsigned int voq_pol_idx[2]; 
	unsigned int fwd_rslt_idx[2]; 
	unsigned short fwd_hash_idx[2]; 
} cs_tunnel_entry_t;
