...
#include <mach/cs75xx_ipc_iplip.h>
...
#define DEFAULT_LIFETIME	"default_lifetime"
...
#define CS752X_NE_DEFAULT_LIFETIME_HELP_MSG "Hash Default Life Time\n" \
			"READ Usage: cat %s\n" \
			"WRITE Usage: echo [value] > %s\n" \
...
static int cs_ne_hash_timer_period_read_proc(char *buf, char **start,
		off_t offset, int count, int *eof, void *data)
{
	u32 len = 0;

	len += sprintf(buf + len, CS752X_NE_HASH_TIMER_PERIOD_HELP_MSG,
			HASH_TIMER_PERIOD, HASH_TIMER_PERIOD);
	len += sprintf(buf + len, "\n%s = %d\n", HASH_TIMER_PERIOD,
...
static int cs_ne_hash_timer_period_write_proc(struct file *file,
		const char *buffer, unsigned long count, void *data)
{
	char buf[32];
	unsigned long mask;
...
static int cs_ne_default_lifetime_read_proc(char *buf, char **start,
		off_t offset, int count, int *eof, void *data)
{
	u32 len = 0;

	len += sprintf(buf + len, CS752X_NE_DEFAULT_LIFETIME_HELP_MSG,
...
static int cs_ne_default_lifetime_write_proc(struct file *file,
		const char *buffer, unsigned long count, void *data)
{
	char buf[32];
	unsigned long mask;
...
HW_ACCEL_INVAL_EXIT:
	printk(KERN_WARNING "Invalid argument\n");
	printk(KERN_WARNING CS752X_NE_DEFAULT_LIFETIME_HELP_MSG,
			DEFAULT_LIFETIME, DEFAULT_LIFETIME);
	return count;
	/* if we return error code here, PROC fs may retry up to 3 times. */
}
...
//--BUG#40328
	...
	remove_proc_entry(DEFAULT_LIFETIME, proc_driver_cs752x_ne_core);
...
//-- For WFO
	...
	cs752x_add_proc_handler(FE_DOUBLE_CHECK, cs_fe_double_check_read_proc,
				...
				proc_driver_cs752x_ne_fe);
#endif
	cs752x_add_proc_handler(HASH_TIMER_PERIOD,
				cs_ne_hash_timer_period_read_proc,
				cs_ne_hash_timer_period_write_proc,
				proc_driver_cs752x_ne_fe);
	...
	cs752x_add_proc_handler(CORE_LOGIC_DEBUG,
				...
				proc_driver_cs752x_ne_core);
	cs752x_add_proc_handler(DEFAULT_LIFETIME,
				cs_ne_default_lifetime_read_proc,
				cs_ne_default_lifetime_write_proc,
				proc_driver_cs752x_ne_core);
