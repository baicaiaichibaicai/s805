...
#include <mach/cs75xx_qos.h>        //BUG#39672: 2. QoS
...
#if	defined(CONFIG_FTG_1000) || defined(CONFIG_FTW_1000) || defined(CONFIG_FTW_1000_V2) || defined(CONFIG_FTM_100S) || defined(CONFIG_FTM_NICO2)
static const struct port_cfg_info port_cfg_infos[] = {
						 ...
						 int qid, int cnt)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		if (skb == NULL) {
			...
			{
				skb = netdev_alloc_skb(dev, SKB_PKT_LEN + 0x100 + NET_IP_ALIGN);
				if (NET_IP_ALIGN && skb)
		...
		if ( use_internal_buff == 0 && ni_rx_noncache == 0 && is_skb_new_alloc == 1){  //QM_ACP is not enabled if use external DDR
...
#define CS_MOD_BRIDGE_SWID (CS_SWID64_MASK(CS_SWID64_MOD_ID_BRIDGE) | 0x425249444745)
/* try fast bridgine, cxc */
static int cs752x_fast_bridging(struct sk_buff *skb)
{
	...
	br_fdb_da = __br_fdb_get(br_port->br, da);
	if (br_fdb_da == NULL)
		goto no_hash_1;

	br_fdb_sa = __br_fdb_get(br_port->br, sa);
	if (br_fdb_sa == NULL)
		goto no_hash_2;
...
#if 0
	if (dst_nic->netdev_ops->ndo_start_xmit == cs_ni_start_xmit) {
	...
	while ((next_link != hw_wr_ptr) && (done < budget)) {
		...
		if (likely(qm_acp_enabled == 0)) {
			...
			if (next_link == hw_wr_ptr) {
				...
				u32 hw_wr_ptr_cur;
				hw_wr_ptr_cur = (readl(NI_TOP_NI_CPUXRAM_CPU_STA_RX_0 + (instance * 24)));
...
//#endif
	 ...
	 */
		...
		voq = xram_cpu_hdr0.bits.cpuhdr_voq;
		...
		if (((xram_cpu_hdr0.bits.cpuhdr_voq == CPU_PORT7_VOQ_BASE) &&
		...
		if (packet_type == CS_APP_PKT_WFO) {
			if (cs_hw_accel_wfo_handle_rx((xram_cpu_hdr0.bits.cpuhdr_voq == CPU_PORT7_VOQ_BASE) ?
...
#if 0		/*because there is no special action here , so we skip code !!*/
			if (cs_hw_accel_tunnel_handle_rx((xram_cpu_hdr0.bits.cpuhdr_voq == CPU_PORT7_VOQ_BASE) ?
					CS_NI_INSTANCE_WFO_802_3 : instance, voq, skb) != 1)
				/*
				* wfo already process skb if it return non 1
				*/
...
SKB_HANDLED:
	...
	//cpuxram_pkt_cnt[instance] = readl(NI_TOP_NI_CPUXRAM_PKT_CNT_0 +
	//		(instance * 24));

	writel(next_link, NI_TOP_NI_CPUXRAM_CPU_CFG_RX_0 + (instance * 24));
...
{
	mac_info_t * tp = netdev_priv(napi->dev);
	int instance = tp->irq - IRQ_NI_RX_XRAM0;

	return cs_ni_poll_dev(instance, NULL, napi, budget);
}
#endif
...
{
	return cs_ni_poll_dev(3, napi->dev, napi, budget);
}

static int cs_ni_poll_4(struct napi_struct *napi, int budget)
{
	return cs_ni_poll_dev(4, napi->dev, napi, budget);
}
		...
		ni_header_a_0_t *header_a0, ni_header_a_1_t *header_a1, u8 qid)
{
	...
	int frag_id = 0, len, total_len;
	char *pkt_datap;

	ni = &ni_private_data;

	if ((skb == NULL) || (header_a0 == NULL) || (header_a1 == NULL))
		return -1;

	if (qid >= NI_DMA_LSO_TXQ_NUM)
		return -1; /* FIXME! a better return value? or better handle? */
	...
	while (snd_pages != 0) {
		...
		curr_desc->word0.bits32 = (u32)len;
		curr_desc->word2.bits32 = 0;
		curr_desc->word3.bits32 = set_desc_word3_calc_l4_chksum(skb,
				dev, total_len, frag_id);
		...
		if( cs_acp_enable & CS75XX_ACP_ENABLE_NI){
			curr_desc->word1.bits32 = virt_to_phys(pkt_datap)|\
				GOLDENGATE_ACP_BASE;
		}
		else{
#endif
		curr_desc->word1.bits32 = (u32)dma_map_single(NULL,
				(void *)pkt_datap, len, DMA_TO_DEVICE);
#ifdef CONFIG_CS752X_PROC
		}
#endif

		if (snd_pages == 1) {
			curr_desc->word0.bits32 |= EOF_BIT; /* EOF */
			if (total_len < 64)
				curr_desc->word3.bits32 |= LSO_IP_LENFIX_EN;
		...
		if (frag_id == 0) {
			curr_desc->word0.bits32 |= SOF_BIT; /* SOF */
			curr_desc->word2.bits32 = (total_len << 16) &
				0xFFFF0000;
			/* Enable LSO Debug:
...
#ifdef CONFIG_CS752X_PROC
			if ((dev != NULL) && (total_len > (dev->mtu + 14)) &&
		...
		curr_desc->word4.bits32 = header_a0->bits32;
		curr_desc->word5.bits32 = header_a1->bits32;
	...
	writel(swtxq->wptr, swtxq->wptr_reg);
...
int ni_special_start_xmit(struct sk_buff *skb, HEADER_A_T *header_a)
{
		...
		struct net_device * dev;
		int snd_pages;
		int frag_id = 0, len, total_len, lso_tx_qid;
...
#ifdef CONFIG_CS752X_HW_ACCELERATION_IPSEC
		CS_IPSEC_RE_VOQ_ID_FROM_CPU(tx_queue); /* for outbound IPSec */
#else
		tx_queue = ENCAPSULATION_VOQ_BASE;
#endif
...
#ifdef CONFIG_CS752X_HW_ACCELERATION
	cs_qos_set_voq_id_to_skb_cs_cb(skb, dev, QUEUE_PER_INSTANCE - 1);
#endif
	return (QUEUE_PER_INSTANCE - 1);
}
#endif
...
#if defined(CONFIG_CORTINA_CUSTOM_BOARD) || defined(CONFIG_CORTINA_FUTURE)
...
#else if defined(CONFIG_CORTINA_BHR)
...
static void cs_ne_init_cfg(void)
{
	...
	reg_val = readl(GLOBAL_GLOBAL_CONFIG);
	reg_val |= (0x40000);
	writel(reg_val, GLOBAL_GLOBAL_CONFIG);
#endif
...
static inline void cs_ni_reset_tx_ring(void)
{
	...
	for (i = 0; i < NI_DMA_LSO_TXQ_NUM; i++) {
		...
		writel(rptr_reg.bits32, swtxq->wptr_reg);
...
#endif
#else
	writel(ni->intr_cpuxram_rxpkt_mask, NI_TOP_NI_CPUXRAM_RXPKT_INTENABLE_0);
#endif
...
static irqreturn_t cs_ni_tx_interrupt(int irq, void *dev_instance)
{
	...
	status = readl(DMA_DMA_LSO_TXQ0_INTERRUPT + tx_qid * 8);
	if (status != 0) {
		writel(0, DMA_DMA_LSO_TXQ0_INTENABLE + tx_qid * 8);
		writel(status, DMA_DMA_LSO_TXQ0_INTERRUPT + tx_qid * 8);
		dev = get_dev_from_lso_txqid(tx_qid);
		cs_dma_tx_complete(tx_qid, dev);
		writel(0x7, DMA_DMA_LSO_TXQ0_INTENABLE + tx_qid * 8);
	}
...
#warning CUSTOM_BOARD_FIX_ME
	...
	defined(CONFIG_CORTINA_ENGINEERING) || defined(CONFIG_CORTINA_ENGINEERING_S) || \
...
#else	/* CONFIG_CORTINA_CUSTOM_BOARD */
	...
	if (tp->port_id == GE_PORT2)
		netif_carrier_on(dev);
#endif /* CONFIG_CORTINA_PON */
#endif /* CONFIG_CORTINA_CUSTOM_BOARD */
...
#if 0
/* modified version from Wen, this is to use a common API to fill knownn info
 * to DMA TXQ descriptor */
static int cs_ni_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	ni_info_t *ni;
	struct net_device *org_dev = dev, *xmit_dev = dev;
	mac_info_t *org_tp = netdev_priv(org_dev);
	mac_info_t *xmit_tp = netdev_priv(xmit_dev);
	ni_header_a_0_t ni_header_a0;
	ni_header_a_1_t ni_header_a1;
	struct netdev_queue *org_txq, *xmit_txq;
	int status, lso_tx_qid, tx_qid = 0;
	u16 tx_queue = 0;
#ifdef CONFIG_CS752X_VIRTUAL_NETWORK_INTERFACE
	int err_virt_ni = 0;
#endif
	...
	tx_qid = skb_get_queue_mapping(skb);
#endif /* CONFIG_NET_SCH_MULTIQ */
	org_txq = xmit_txq = netdev_get_tx_queue(org_dev, tx_qid);

	lso_tx_qid = get_dma_lso_txqid(org_dev);

	if (lso_tx_qid >= NI_DMA_LSO_TXQ_NUM)
		return 0; /* FIXME! a better return value? or better handle? */

	ni_header_a0.bits32 = HDRA_CPU_PKT;
	ni_header_a1.bits32 = 0;

#ifdef CONFIG_CS752X_VIRTUAL_NETWORK_INTERFACE
	err_virt_ni = cs_ni_virt_ni_process_tx_skb(skb, org_dev, txq);
	if (err_virt_ni < 0) {
		dev_kfree_skb(skb);
	...
	xmit_dev = skb->dev;
	xmit_tp = netdev_priv(xmit_dev);
	xmit_txq = netdev_get_tx_queue(xmit_dev, tx_qid);
	/* in case we introduce any new send_page */
	snd_pages = skb_shinfo(skb)->nr_frags + 1;
#endif

	/* get the destination VOQ */
	switch (xmit_tp->port_id) {
	case GE_PORT0:
		tx_queue = GE_PORT0_VOQ_BASE;
		break;
	case GE_PORT1:
	...
	if (ni->neEnabled) {
		cs_cb = CS_KERNEL_SKB_CB(skb);
		cs_cb->common.egress_port_id = xmit_tp->port_id;
		if (!(cs_cb->common.module_mask & CS_MOD_MASK_QOS)) {
			cs_cb->action.voq_pol.d_voq_id = tx_queue;
		}
		cs_core_logic_add_connections(skb);
		/* anything else? */
...
#endif
	ni_header_a0.bits.dvoq = tx_queue;
	status = ni_xmit_dma_lso_txq(skb, xmit_dev, &ni_header_a0,
			&ni_header_a1, lso_tx_qid);
	if (status == NETDEV_TX_BUSY) {
		if ((org_txq != NULL) && (!netif_tx_queue_stopped(org_txq))) {
			netif_tx_stop_queue(org_txq);
			/* This is a hard error, log it. */
			printk("BUG! Tx Ring full when queue awake!\n");
		}
	}
	...
	/* status == packet sent */
	xmit_tp->ifStatics.tx_packets += status;
	xmit_tp->ifStatics.tx_bytes += skb->len;
	if (xmit_tp != org_tp) {
		org_tp->ifStatics.tx_packets += status;
		org_tp->ifStatics.tx_bytes += skb->len;
	}
	...
	txq_trans_update(xmit_txq);
	if (xmit_txq != org_txq)
		txq_trans_update(org_txq);
...
static int cs_ni_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (free_desc <= snd_pages) {
		...
		//isPtr->tx_dropped++;
		printk("No available descriptor! free desc = %d, snd_pages = "
				"%d\n", free_desc, snd_pages);
		printk("wptr = %d, rptr = %d, finished_idx = %d\n", swtxq->wptr,
				rptr, swtxq->finished_idx);
...
static int cs_ni_set_features(struct net_device *netdev, u32 features)
{
...
static int cs_ni_init_module_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < 5; i++) {
		...
		netif_napi_add(ni_private_data.dev[i], &tp->napi,
...
/* 3.4.11 Change for register as platform device */
static void cs_ni_cleanup_module_exit(struct platform_device *pdev)
{
	int i;
	ni_info_t *ni;
	mac_info_t *tp;
