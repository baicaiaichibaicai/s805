...
#define DRV_VERSION	__stringify(3.19.1) DRIVERIOV DRV_HW_PERF FPGA \
			VMDQ_TAG BYPASS_TAG
...
int upe_flag[NR_ETH] = {0};
static struct net_dev_hw_stats * ixgbe_get_hw_stats(struct net_device *netdev);
static int ixgbe_get_link_status(struct net_device *netdev);
static int ixgbe_get_link_speed(struct net_device *netdev);
static int ixgbe_get_link_duplex(struct net_device *netdev);
static void ixgbe_set_link_up_phy(struct net_device *netdev);
static void ixgbe_set_link_down_phy(struct net_device *netdev);
static void ixgbe_set_power_up_phy(struct net_device *netdev);
static void ixgbe_set_power_down_phy(struct net_device *netdev);
void ixgbe_txbulk_timer(struct ixgbe_adapter *adapter);
netdev_tx_t ixgbe_xmit_frame_fast(struct sk_buff *skb, struct net_device *netdev);
static void ixgbe_get_pci_bridge_name(struct pci_dev *pdev);
#endif
...
#ifdef CONFIG_FERRET 
		/* Don't delete. it's TX ring to unmapped the DMA */
		tx_desc->wb.status = 0;
#endif
		tx_buffer++;
		tx_desc++;
				   ...
				   struct ixgbe_rx_buffer *bi)
{
	...
	if (likely(!skb)) {
		...
		skb = netdev_alloc_skb_ip_align(netdev_ring(rx_ring),
						rx_ring->rx_buf_len + IXGBE_82599_RCL_PAD);
#else
		skb = netdev_alloc_skb_ip_align(netdev_ring(rx_ring),
						rx_ring->rx_buf_len);
#endif
...
#if defined(CONFIG_NETDEVICES_MULTIQUEUE) && defined(CONFIG_FERRET)
			       ...
			       int budget)
{
	...
	u16 len = 0;
	u16 cleaned_count = ixgbe_desc_unused(rx_ring);
//#if defined(CONFIG_FERRET) && defined(CONFIG_NETDEVICES_MULTIQUEUE)
//	IXGBE_WRITE_REG(&q_vector->adapter->hw, IXGBE_RSSRK(1), q_vector->adapter->rss_key_index++);
//#endif
	...
	do {
		...
		if (fast_nic_flag) {
			total_rx_bytes -= 4;
			rx_ring->pkt_buket[total_rx_packets] = skb;
		}
		else
#endif
		ixgbe_rx_skb(q_vector, rx_ring, rx_desc, skb);
...
#if defined(CONFIG_NETDEVICES_MULTIQUEUE) && defined(CONFIG_FERRET)
	if (fast_nic_flag){
		rx_ring->pkt_buket[total_rx_packets] = NULL;
		rx_ring->pkt_cnt = total_rx_packets;
	}
#endif
	rx_ring->stats.packets += total_rx_packets;
	rx_ring->stats.bytes += total_rx_bytes;
...
#if defined(CONFIG_NETDEVICES_MULTIQUEUE) && defined(CONFIG_FERRET)
		if (fast_nic_flag) {
			for(n=0; n<ring->pkt_cnt; n++) 
				netif_receive_skb(ring->pkt_buket[n]);
		}
#endif
	}
...
#if defined(CONFIG_FERRET) && defined(CONFIG_NETDEVICES_MULTIQUEUE)
		cpus_clear(cpu_mask);
		cpu_set(vector, cpu_mask);
		irq_set_affinity(adapter->msix_entries[vector].vector, &cpu_mask);
#endif
...
#ifdef CONFIG_FERRET
	} else if (max_frame <= IXGBE_RXBUFFER_1536) {
...
#if defined(CONFIG_FERRET) && defined(CONFIG_NETDEVICES_MULTIQUEUE)
	fdir = min_t(int, IXGBE_MAX_FDIR_INDICES, num_online_cpus()/num_online_nodes());
#else
...
#ifdef CONFIG_FERRET
	spin_lock_init(&tx_ring->xmit_lock);
	skb_queue_head_init(&tx_ring->tx_queue_list);
#endif
...
#ifdef CONFIG_FERRET
	adapter->hw_stats.prc64 = adapter->stats.prc64;
	adapter->hw_stats.prc127 = adapter->stats.prc127;
	adapter->hw_stats.prc255 = adapter->stats.prc255;
	adapter->hw_stats.prc511 = adapter->stats.prc511;
	adapter->hw_stats.prc1023 = adapter->stats.prc1023;
	adapter->hw_stats.prc1522 = adapter->stats.prc1522;
#endif
...
static void ixgbe_watchdog_update_link(struct ixgbe_adapter *adapter)
{
...
#ifdef CONFIG_FERRET
	extern int linkpack_enable;
	if (!linkpack_enable && !(adapter->flags & IXGBE_FLAG_NEED_LINK_UPDATE))
		return;
#else
	if (!(adapter->flags & IXGBE_FLAG_NEED_LINK_UPDATE))
		return;
	...
	if(!link_up || !netif_carrier_ok(adapter->netdev)){
		linkpack_check(adapter->port_num, link_up);
	}
#endif
...
static void ixgbe_watchdog_link_is_up(struct ixgbe_adapter *adapter)
{
	...
	e_info(drv, "NIC Link is Up %s, Flow Control: %s\n",
	       ...
	       (flow_rx ? "RX" :
			(flow_tx ? "TX" : "None"))));
	sprintf(buf,"ixgbe %s: NIC Link is Up %s, Flow Control: %s\n", netdev->name,
			(link_speed == IXGBE_LINK_SPEED_10GB_FULL ?
			 "10 Gbps" :
			 (link_speed == IXGBE_LINK_SPEED_1GB_FULL ?
			  "1 Gbps" :
			...
			((flow_rx && flow_tx) ? "RX/TX" :
			 (flow_rx ? "RX" :
			  (flow_tx ? "TX" : "None"))));
	if (strlen(buf))
		system_log_general(NULL, netdev->name,0, NULL, LOG_SYSTEM_IFACE_STATUS, buf);
...
static void ixgbe_watchdog_link_is_down(struct ixgbe_adapter *adapter)
{
	...
	sprintf(buf, "ixgbe %s: NIC Link is Down\n", netdev->name);
	if (strlen(buf))
		system_log_general(NULL, netdev->name,0, NULL, LOG_SYSTEM_IFACE_STATUS, buf);
	netif_carrier_off(netdev);
	netif_tx_stop_all_queues(netdev);
...
#if defined(CONFIG_NETDEVICES_MULTIQUEUE) && defined(CONFIG_FERRET)
	if (fast_nic_flag)
		return ixgbe_xmit_frame_fast(skb, netdev);
#endif
...
#endif
#ifdef CONFIG_FERRET
	.get_hw_stats		= ixgbe_get_hw_stats,
	.get_link_status	= ixgbe_get_link_status,
	.get_link_speed		= ixgbe_get_link_speed,
	.get_link_duplex	= ixgbe_get_link_duplex,
	.set_link_up_phy    = ixgbe_set_link_up_phy,
	.set_link_down_phy  = ixgbe_set_link_down_phy,
	.set_power_up_phy   = ixgbe_set_power_up_phy,
	.set_power_down_phy = ixgbe_set_power_down_phy,
#endif
};
...
#ifdef CONFIG_FERRET
	indices = min_t(unsigned int, MAX_TX_QUEUES, num_online_cpus()/num_online_nodes());
#endif
...
#ifdef CONFIG_FERRET
	sprintf(netdev->name, "eth%d", e1000_port[cards_found]);
	adapter->port_num = e1000_port[cards_found];
	ixgbe_get_pci_bridge_name(pdev);
	netdev_global[adapter->port_num] = netdev;
#else
...
static int ixgbe_get_link_status(struct net_device *netdev)
{
	...
	if (adapter->hw.mac.ops.check_link)
...
static void ixgbe_set_link_up_phy(struct net_device *dev)
{
	if(!dev->link_down_phy)
		return;
	dev->link_down_phy = 0;
	netif_carrier_on(dev);
}

static void ixgbe_set_link_down_phy(struct net_device *dev)
{
	netif_carrier_off(dev);
	dev->link_down_phy = 1;
}

static void ixgbe_set_power_up_phy(struct net_device *dev)
{
	struct ixgbe_adapter *adapter = netdev_priv(dev);
	struct ixgbe_hw *hw = &adapter->hw;
	uint32_t data;
	...
	dev->link_down_phy = 0;
	data = IXGBE_READ_REG(hw, IXGBE_AUTOC);
	data &= ~(0x0F<<12);
	data |= (0x07<<12);
	IXGBE_WRITE_REG(hw, IXGBE_AUTOC, data);

	netif_carrier_on(dev);
}

static void ixgbe_set_power_down_phy(struct net_device *dev)
{
	struct ixgbe_adapter *adapter = netdev_priv(dev);
	struct ixgbe_hw *hw = &adapter->hw;
	uint32_t data;
...
netdev_tx_t ixgbe_xmit_frame_ring_fast(struct ixgbe_ring *tx_ring)
{
	struct ixgbe_tx_buffer *tx_buffer_info;
	union ixgbe_adv_tx_desc *tx_desc;
	struct sk_buff *skb;
	...
	if (ixgbe_maybe_stop_tx(tx_ring, tx_ring->pkt_cnt+4))
		return NETDEV_TX_BUSY;

	while((skb=tx_ring->pkt_buket[n++]))
	{
		dma_addr_t dma;
		...
		dma = dma_map_single(tx_ring->dev, skb->data, tx_ring->rx_buf_len, DMA_TO_DEVICE);
		if (dma_mapping_error(tx_ring->dev, dma))
			goto dma_error;

		tx_ring->pkt_buket[n-1] = NULL;
		cmd_type = cpu_to_le32(IXGBE_ADVTXD_DTYP_DATA | IXGBE_ADVTXD_DCMD_IFCS | IXGBE_ADVTXD_DCMD_DEXT);

		tx_buffer_info = &tx_ring->tx_buffer_info[tx_ring->next_to_use];
		dma_unmap_addr_set(tx_buffer_info, dma, dma);
		tx_buffer_info->bytecount = skb->len;
		...
		tx_desc->read.buffer_addr = cpu_to_le64(dma);
		tx_desc->read.cmd_type_len = cmd_type | cpu_to_le32(skb->len);
		tx_desc->read.olinfo_status = cpu_to_le32(skb->len << IXGBE_ADVTXD_PAYLEN_SHIFT);
		tx_desc->read.cmd_type_len |= cpu_to_le32(IXGBE_TXD_CMD_EOP | IXGBE_TXD_CMD_RS);
...
void ixgbe_timeout_txbulk_fast(unsigned long data) 
{
	struct ixgbe_adapter *adapter = (struct ixgbe_adapter *)data;
	struct ixgbe_ring *tx_ring;
	int n, k;
	...
	if (fast_nic_flag){
		if(unlikely(adapter->hlreg0 & IXGBE_HLREG0_RXCRCSTRP)){
			adapter->hlreg0 = IXGBE_READ_REG(&adapter->hw, IXGBE_HLREG0);
			adapter->hlreg0 &= ~IXGBE_HLREG0_RXCRCSTRP;
			IXGBE_WRITE_REG(&adapter->hw, IXGBE_HLREG0, adapter->hlreg0);

#ifndef CONFIG_FERRET
			adapter->netdev->features &= ~(NETIF_F_RXCSUM | NETIF_F_RXHASH | NETIF_F_GRO);
#else
			adapter->netdev->features &= ~(NETIF_F_RXCSUM | NETIF_F_RXHASH);
#endif
			adapter->netdev->features &= ~(NETIF_F_SCTP_CSUM | NETIF_F_IP_CSUM);
			adapter->netdev->features = adapter->netdev->netdev_ops->ndo_fix_features(adapter->netdev, adapter->netdev->features );
		}
	}
	else{
		if(!(adapter->hlreg0 & IXGBE_HLREG0_RXCRCSTRP)){
			adapter->hlreg0 = IXGBE_READ_REG(&adapter->hw, IXGBE_HLREG0);
			adapter->hlreg0 |= IXGBE_HLREG0_RXCRCSTRP;
			IXGBE_WRITE_REG(&adapter->hw, IXGBE_HLREG0, adapter->hlreg0);
			...
			adapter->netdev->features 
				= adapter->netdev->netdev_ops->ndo_fix_features(adapter->netdev, adapter->netdev->features );
		}
		mod_timer(&adapter->tx_timer, jiffies+HZ);
		return;
	}
	...
	for(n=0; n<adapter->num_tx_queues; n++){
		...
		spin_lock(&tx_ring->xmit_lock);
		queue_len = skb_queue_len(&tx_ring->tx_queue_list);
		if(queue_len && time_after(jiffies, tx_ring->trans_start+2))
		{
			int tx_len = ixgbe_desc_unused(tx_ring)-4;

			tx_len = (tx_len > 120)? 120 : tx_len;
			tx_len = (tx_len > queue_len)? queue_len : tx_len; 
			for(k=0; k<tx_len; k++)
				tx_ring->pkt_buket[k] = __skb_dequeue(&tx_ring->tx_queue_list);;
			tx_ring->pkt_buket[k] = NULL; 
			tx_ring->pkt_cnt = tx_len;

			ixgbe_clean_tx_irq(tx_ring->q_vector, tx_ring);
			ixgbe_xmit_frame_ring_fast(tx_ring);

			for(k=0; k<tx_ring->pkt_cnt; k++){
				if (unlikely(tx_ring->pkt_buket[k])){
					tx_ring->tx_stats.tx_busy++;
					kfree_skb(tx_ring->pkt_buket[k]);
				}
			}
		}
		spin_unlock(&tx_ring->xmit_lock);
	}
	mod_timer(&adapter->tx_timer, jiffies+1);
}

void ixgbe_txbulk_timer(struct ixgbe_adapter *adapter) 
{
	if(adapter->tx_timer.function)
		return;

	init_timer(&adapter->tx_timer);
	adapter->tx_timer.expires = jiffies+HZ;
	adapter->tx_timer.function = ixgbe_timeout_txbulk_fast;
	adapter->tx_timer.data = (unsigned long)adapter;
	add_timer(&adapter->tx_timer);
}
...
netdev_tx_t ixgbe_xmit_frame_fast(struct sk_buff *skb, struct net_device *netdev)
{
	...
	if(unlikely(skb->queue_mapping >= adapter->num_tx_queues)) 
		skb->queue_mapping = 0;
	...
	queue_len = skb_queue_len(&tx_ring->tx_queue_list);
	if(likely(queue_len < 100000))
		__skb_queue_tail(&tx_ring->tx_queue_list, skb);
	else{
		tx_ring->tx_stats.tx_busy++;
		dev_kfree_skb(skb);
		spin_unlock(&tx_ring->xmit_lock);
	...
	tx_len = ixgbe_desc_unused(tx_ring)-4;

	if((tx_len<64) || (queue_len < 64)) {
		tx_ring->trans_start = jiffies;
		spin_unlock(&tx_ring->xmit_lock);
		return NETDEV_TX_OK;
	}   
	...
	tx_len = 64;
	for(n=0; n<tx_len; n++)
		tx_ring->pkt_buket[n] = __skb_dequeue(&tx_ring->tx_queue_list);;

	tx_ring->pkt_buket[n] = NULL; 
	tx_ring->pkt_cnt = tx_len;
	ixgbe_xmit_frame_ring_fast(tx_ring);
	for(n=0; n<tx_ring->pkt_cnt; n++){
		if(tx_ring->pkt_buket[n]){
			tx_ring->tx_stats.tx_busy++;
			kfree_skb(tx_ring->pkt_buket[n]);
		}
	}
	tx_ring->pkt_cnt = 0;
	spin_unlock(&tx_ring->xmit_lock);
...
static void ixgbe_get_pci_bridge_name(struct pci_dev *pdev)
{
	...
	struct pci_dev *bridge2;
	struct pci_dev *bridge3;
	unsigned short device;

	device = pdev->device;

	strlcpy(g_iface_pci[e1000_port[cards_found]].name, pci_name(pdev), 13);
	g_iface_pci[e1000_port[cards_found]].device = device;

	bridge = pdev->bus->self;
	if(!bridge)
		return;
	strlcpy(g_iface_pci[e1000_port[cards_found]].bridge, pci_name(bridge), 13);

	bridge2 = bridge->bus->self;
	if(!bridge2)
		return;

	bridge3 = bridge2->bus->self;
	if(!bridge3)
		return;
	strlcpy(g_iface_pci[e1000_port[cards_found]].bridge2, pci_name(bridge3), 13);
}

extern struct proc_dir_entry *proc_ferret_system;

static int ixgbe_upe_proc_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int i, offset = 0;

	for(i=0;i<NR_ETH;i++) {
		if(upe_flag[i])
			offset += snprintf(page+offset, count-offset, "eth%d upe on\n", i);
	}
	return offset;
}

static int ixgbe_upe_proc_write(struct file *file, const char __user *buffer, unsigned long count, void *data)
{
	char tmp[8] = {0};
	int num, flag;
	u32 fctrl;
	...
	if (count > 8) // Invalid input
		return count;
	sscanf(tmp, "%d %d\n", &num, &flag);

	if ((num < 0) || (num >= NR_ETH)) // Invalid interface number
		return count;

	if ((flag < 0) || (flag > 2)) // Invalid flag
		return count;

	dev = netdev_global[num];
	if (dev == NULL)
		return count;
...
static int ixgbe_upe_proc_init(void)
{
	struct proc_dir_entry *p = NULL;

	p = create_proc_entry("ixgbe_upe", 0600, proc_ferret_system);
	if (p == NULL) {
		printk("Creating proc entry failed: /proc/ferret/system/ixgbe_upe\n");
		return -ENOMEM;
	}

	p->read_proc = ixgbe_upe_proc_read;
	p->write_proc = ixgbe_upe_proc_write;

	return 0;
}
module_init(ixgbe_upe_proc_init)
#endif
/* ixgbe_main.c */
