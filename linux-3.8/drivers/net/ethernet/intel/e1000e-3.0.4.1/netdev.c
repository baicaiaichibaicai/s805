...
#ifdef CONFIG_FERRET
static int e1000e_get_link_status(struct net_device *netdev);
static int e1000e_get_link_speed(struct net_device *netdev);
static int e1000e_get_link_duplex(struct net_device *netdev);
static void e1000e_set_link_up_phy(struct net_device *dev);
static void e1000e_set_link_down_phy(struct net_device *dev);
static void e1000e_set_power_up_phy(struct net_device *dev);
static void e1000e_set_power_down_phy(struct net_device *dev);
static struct net_dev_hw_stats * e1000_get_hw_stats(struct net_device *netdev);
extern int e1000_port[];
int cards_found = 0;
extern struct iface_pci_name g_iface_pci[]; 
EXPORT_SYMBOL(e1000_port);
EXPORT_SYMBOL(cards_found);
#endif
				   ...
				   int cleaned_count, gfp_t gfp)
{
	...
	while (cleaned_count--) {
		...
		skb = __netdev_alloc_skb_ip_align(netdev, bufsz + E1000E_RCL_PAD, gfp);
#else
...
static int e1000_close(struct net_device *netdev)
{
	...
	if (!test_bit(__E1000_DOWN, &adapter->state)) {
		...
		sprintf(buf, "%s NIC Link is Down", adapter->netdev->name);
#ifdef CONFIG_FERRET
		if (strlen(buf))
			system_log_general(NULL, adapter->netdev->name,0, NULL, LOG_SYSTEM_IFACE_STATUS, buf);
#endif
...
static void e1000_print_link_info(struct e1000_adapter *adapter)
{
	...
	sprintf(buf, "%s NIC Link is Up %d Mbps %s Duplex, Flow Control: %s\n",
			adapter->netdev->name, adapter->link_speed,
			adapter->link_duplex == FULL_DUPLEX ? "Full" : "Half",
			(ctrl & E1000_CTRL_TFCE) && (ctrl & E1000_CTRL_RFCE) ? "Rx/Tx" :
			(ctrl & E1000_CTRL_RFCE) ? "Rx" :
...
{
#ifdef CONFIG_FERRET
	struct net_device *netdev = pci_get_drvdata(to_pci_dev(&pdev->dev));
	if(!netdev)
		return;
#endif
...
#endif /* HAVE_NDO_SET_FEATURES */
#ifdef CONFIG_FERRET
	.get_link_status	= e1000e_get_link_status,
	.get_link_speed		= e1000e_get_link_speed,
	.get_link_duplex	= e1000e_get_link_duplex,
	.set_link_up_phy    = e1000e_set_link_up_phy,
	.set_link_down_phy  = e1000e_set_link_down_phy,
	.set_power_up_phy   = e1000e_set_power_up_phy,
	.set_power_down_phy = e1000e_set_power_down_phy,
	.get_hw_stats		= e1000_get_hw_stats,
#endif
};
...
#endif
{
	...
	g_iface_pci[e1000_port[cards_found]].device = pdev->device;
...
static int e1000e_get_link_speed(struct net_device *netdev)
{
	...
	switch (hw->phy.media_type)
	{
		...
		case e1000_media_type_fiber:
		case e1000_media_type_internal_serdes:
			e1000e_get_speed_and_duplex_fiber_serdes(hw, &speed, &duplex);
			break;
		default:
		case e1000_media_type_unknown:
			break;
...
static void e1000e_set_power_up_phy(struct net_device *dev)
{
	...
	if (adapter->hw.phy.media_type == e1000_media_type_copper) {
		u16 mii_reg = 0;
		e1e_rphy(&adapter->hw, MII_BMCR, &mii_reg);
		mii_reg &= ~(BMCR_PDOWN | BMCR_ISOLATE);
		e1e_wphy(&adapter->hw, MII_BMCR, mii_reg);
		e1e_rphy(&adapter->hw, MII_BMCR, &mii_reg);
	}
	else{
		u32 mii_reg;
		mii_reg = E1000_READ_REG(&adapter->hw, E1000_CTRL);
		mii_reg &= ~E1000_CTRL_LRST;
		mii_reg &= ~E1000_CTRL_PHY_RST;
...
static void e1000e_set_power_down_phy(struct net_device *dev)
{
	...
	if (adapter->hw.phy.media_type == e1000_media_type_copper) {
		...
		u32 mii_reg, txcw;
		mii_reg = E1000_READ_REG(&adapter->hw, E1000_CTRL);
		txcw = E1000_READ_REG(&adapter->hw, E1000_TXCW);
		...
		E1000_WRITE_REG(&adapter->hw, E1000_CTRL, mii_reg);
		udelay(10000);
		E1000_WRITE_REG(&adapter->hw, E1000_TXCW, txcw);
		mii_reg = E1000_READ_REG(&adapter->hw, E1000_CTRL);
		txcw = E1000_READ_REG(&adapter->hw, E1000_TXCW);
	}
	netif_carrier_off(dev);
