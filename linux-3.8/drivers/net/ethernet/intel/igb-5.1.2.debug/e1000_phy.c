...
s32 e1000_read_phy_reg_mdic(struct e1000_hw *hw, u8 addr, u32 offset, u16 *data)
#else
 ...
 **/
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
s32 e1000_write_phy_reg_mdic(struct e1000_hw *hw, u8 addr, u32 offset, u16 data)	
#else
...
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
	ret_val = e1000_read_phy_reg_mdic(hw, hw->phy.addr,
...
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
	ret_val = e1000_write_phy_reg_mdic(hw, hw->phy.addr, MAX_PHY_REG_ADDRESS & offset,
#else
...
/**
 *  igb_set_page_igp - Set page as on IGP-like PHY(s)
 *  @hw: pointer to the HW structure
 ...
 **/
s32 igb_set_page_igp(struct e1000_hw *hw, u16 page)
{
	DEBUGFUNC("igb_set_page_igp");
	printk("## rsy : func => %s\r\n", __func__);
...
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
	return e1000_write_phy_reg_mdic(hw, hw->phy.addr, IGP01E1000_PHY_PAGE_SELECT, page);
#else
...
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
		ret_val = e1000_write_phy_reg_mdic(hw, hw->phy.addr,				
#else
	...
	if(phy->id == M88E1112_E_PHY_ID){
		...
		return ret_val;
	     ...
	     /* rsy test */
            ...
            if (ret_val)
                return ret_val;
#if 1 /* media converter success */
            data &= ~(0x1 << 12);
            data |= (0x1 << 9) | (0x1 << 8) | (0x1 << 6);
#else /* auto nego enable */
            data |= (0x1 << 12) | (0x1 << 8) | (0x1 << 6);
#endif
            ret_val = phy->ops.write_reg(hw, 0, data);
            if (ret_val)
                return ret_val;
#endif
           ...
           /* write reg 17_3 : LED Polarity */
            ...
            ret_val = phy->ops.write_reg(hw, 17, 0x0055);
            if (ret_val)
                return ret_val;
	    ...
	    timeout = 1000;
            ret_val = phy->ops.write_reg(hw, 0, data | 0x8000);
            if (ret_val)
                return ret_val;
           ...
           /* rsy add for adveritse */
#if 1
            ret_val = phy->ops.write_reg(hw, E1000_M88E1112_PAGE_ADDR, 1);
            if (ret_val)
                return ret_val;

            ret_val = phy->ops.read_reg(hw, 4, &data);
            if (ret_val)
                return ret_val;

            data |= (0x1 << 8) | (0x1 << 7) | (0x1 << 6) | (0x1 << 5);

            ret_val = phy->ops.write_reg(hw, 4, data);
            if (ret_val)
                return ret_val;
...
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
s32 e1000_write_reg_gs40g(struct e1000_hw *hw, u8 addr, u32 offset, u16 data)
#else
...
#endif
{
...
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
	ret_val = e1000_write_phy_reg_mdic(hw, addr, GS40G_PAGE_SELECT, page);
#else
	...
	if (ret_val)
		goto release;
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
	ret_val = e1000_write_phy_reg_mdic(hw, addr, offset, data);
#else
 ...
 **/
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
s32 e1000_read_reg_gs40g(struct e1000_hw *hw, u8 addr, u32 offset, u16 *data)
#else
...
#endif
{
	...
	if (ret_val)
		goto release;
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
	ret_val = e1000_read_phy_reg_mdic(hw, addr, offset, data);
#else
