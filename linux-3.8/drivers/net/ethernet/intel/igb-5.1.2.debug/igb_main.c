...
#include <linux/tcp.h>
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
#include <linux/phy.h>
#endif
#ifdef NETIF_F_TSO
...
#ifdef CONFIG_FERRET
static int igb_get_link_status(struct net_device *netdev);
static int igb_get_link_speed(struct net_device *netdev);
static int igb_get_link_duplex(struct net_device *netdev);
static void igb_set_link_up_phy(struct net_device *dev);
static void igb_set_link_down_phy(struct net_device *dev);
static void igb_set_power_up_phy(struct net_device *dev);
static void igb_set_power_down_phy(struct net_device *dev);
static void igb_txbulk_timer(struct igb_adapter *adapter);
static void igb_timeout_txbulk(unsigned long data);
static struct net_dev_hw_stats * igb_get_hw_stats(struct net_device *netdev);
static void igb_get_pci_bridge_name(struct pci_dev *pdev);
extern int e1000_port[];
extern int cards_found; 
extern int use_e1000_port;
extern struct iface_pci_name g_iface_pci[];
static int nr_i210_copper;
#endif
...
static DEFINE_PCI_DEVICE_TABLE(igb_pci_tbl) = {
	...
	{ PCI_VDEVICE(INTEL, E1000_DEV_ID_I210_UNPROGRAMMED) },
	...
	{ PCI_VDEVICE(INTEL, E1000_DEV_ID_I350_UNPROGRAMMED) },
...
#ifdef CONFIG_FERRET  
	do {
		if ((pdev = pci_get_device(0x8086, 0x1533, pdev)) !=NULL)
			nr_i210_copper++;
	} while (pdev != NULL);

	if((nr_i210_copper == 1 ) && (use_e1000_port == 0))
		cards_found=1;
#endif
...
static int igb_request_msix(struct igb_adapter *adapter)
{
	...
	for (i = 0; i < adapter->num_q_vectors; i++) {
		...
		cpus_clear(cpu_mask);
		cpu_set(q_vector->ring->queue_index, cpu_mask);
...
#if defined(CONFIG_NETDEVICES_MULTIQUEUE) && defined(CONFIG_FERRET)
	adapter->rss_queues = min_t(u32, MAX_Q_VECTORS, (int)num_online_cpus()/num_online_nodes());

	if(adapter->hw.mac.type == e1000_82575) {
		if(adapter->rss_queues > 4)
			adapter->rss_queues = 4;
	}
	else if(adapter->hw.mac.type == e1000_i211) { 
		if(adapter->rss_queues > IGB_MAX_RX_QUEUES_I211) 
			adapter->rss_queues = IGB_MAX_RX_QUEUES_I211; 
	} 
	else if(adapter->hw.mac.type == e1000_i210) {
		if(adapter->rss_queues > 4)
			adapter->rss_queues = 4;
	}
#endif
...
static int igb_enet_mdio_read(struct mii_bus *bus, int mii_id, int regnum)
{
	...
	err = e1000_read_reg_gs40g(hw, mii_id, regnum, &out);
	if (err)
		return err;
	return out;
}
			       ...
			       u16 val)
{
	...
	return e1000_write_reg_gs40g(hw, mii_id, regnum, val);
}
...
#endif
#ifdef CONFIG_FERRET
	.get_link_status	= igb_get_link_status,
	.get_link_speed		= igb_get_link_speed,
	.get_link_duplex	= igb_get_link_duplex,
	.set_link_up_phy    = igb_set_link_up_phy,
	.set_link_down_phy  = igb_set_link_down_phy,
	.set_power_up_phy   = igb_set_power_up_phy,
	.set_power_down_phy = igb_set_power_down_phy,
	.get_hw_stats		= igb_get_hw_stats,
#endif
};
...
#if defined(CONFIG_MZEN)
extern char crypto_mac[20];
#endif
/**
			       ...
			       const struct pci_device_id *ent)
{
	...
	if (igb_read_mac_addr(hw))
		dev_err(pci_dev_to_dev(pdev), "NVM Read Error\n");

#if defined(CONFIG_MZEN) 
	if (*crypto_mac != '\0') {
		int values[6];
		if(sscanf(crypto_mac, "%x:%x:%x:%x:%x:%x",  &values[0], &values[1], &values[2], &values[3], &values[4], &values[5]) == 6) {
			for (i=0; i<6; ++i)
				hw->mac.addr[i] = (char) values[i];
		}
		hw->mac.addr[5] += cards_found;
	}
#endif
	...
	if (nr_i210_copper==1) {
		if(use_e1000_port) {
			...
			if(pdev->device == 0x1533) {
				sprintf(netdev->name, "eth%d", 1- nr_i210_copper);
				adapter->port_num = 1- nr_i210_copper;
				cards_found--;
			} else {
				sprintf(netdev->name, "eth%d", cards_found); // +1 
				adapter->port_num = cards_found;  
			}
		}
	} else {
		sprintf(netdev->name, "eth%d", e1000_port[cards_found]);
	...
	if (E1000_READ_REG(hw, E1000_MDICNFG) & E1000_MDICNFG_EXT_MDIO)
	{
...
bool igb_has_link(struct igb_adapter *adapter)
{
	 ...
	 * false until the igb_check_for_link establishes link
...
static void igb_watchdog_task(struct work_struct *work)
{
...
#ifdef CONFIG_FERRET
	if(!link || !netif_carrier_ok(netdev))
		linkpack_check(adapter->port_num, link);
#endif
	...
	if (link) {
		...
		if (!netif_carrier_ok(netdev)) {
			...
			sprintf(buf, "igb: %s NIC Link is Up %d Mbps %s, "
					"Flow Control: %s\n",
					netdev->name,
					adapter->link_speed,
					adapter->link_duplex == FULL_DUPLEX ?
		...
		if (netif_carrier_ok(netdev)) {
			...
			printk(KERN_INFO "igb: %s NIC Link is Down\n",
					netdev->name);
			sprintf(buf, "igb: %s NIC Link is Down\n",
					netdev->name);
#ifdef CONFIG_FERRET
			if (strlen(buf))
				system_log_general(NULL, netdev->name,0, NULL, LOG_SYSTEM_IFACE_STATUS, buf);
...
#ifdef CONFIG_FERRET
	if(fast_nic_flag && (++tx_ring->xmit_cnt >= 32)) {
                                  ...
                                  struct net_device *netdev)
{
	...
	ret = igb_xmit_frame_ring(skb, tx_ring);
...
//#if defined(CONFIG_NETDEVICES_MULTIQUEUE) && defined(CONFIG_FERRET)
//	struct igb_adapter *adapter = q_vector->adapter;
//	E1000_WRITE_REG_ARRAY(&adapter->hw, E1000_RSSRK(0), 1, adapter->rss_key_index++);
//#endif
				 ...
				 struct igb_rx_buffer *bi)
{
	...
	if (likely(!skb)) {
		...
		skb = netdev_alloc_skb_ip_align(netdev_ring(rx_ring),
				rx_ring->rx_buffer_len + IGB_RCL_PAD);
#else
		skb = netdev_alloc_skb_ip_align(netdev_ring(rx_ring),
				rx_ring->rx_buffer_len);
#endif
...
static int igb_mii_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd)
{
...
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
	if (adapter->hw.phy.media_type != e1000_media_type_copper &&
		    !(E1000_READ_REG(hw, E1000_MDICNFG) & E1000_MDICNFG_EXT_MDIO))
#else
	if (adapter->hw.phy.media_type != e1000_media_type_copper)
#endif
		return -EOPNOTSUPP;
	...
	case SIOCGMIIREG:
		...
		if (hw->mac.type == e1000_i210 || hw->mac.type == e1000_i211) {
			if (e1000_read_reg_gs40g(&adapter->hw, data->phy_id,
						       data->reg_num & 0x1F,
						       &data->val_out))
				return -EIO;
		} else {
...
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
		if (hw->mac.type == e1000_i210 || hw->mac.type == e1000_i211) {
			if (e1000_write_reg_gs40g(hw, data->phy_id,
						data->reg_num & 0x1F,
						data->val_in))
				return -EIO;
		} else {
...
static void igb_timeout_txbulk(unsigned long data)
{
	...
	if (fast_nic_flag){
		for(n=0; n<adapter->num_tx_queues; n++){
			...
			if(ring && ring->xmit_cnt){
				spin_lock(&ring->xmit_lock);
				writel(ring->next_to_use, ring->tail);          //Transmit Descriptor Tail
				spin_unlock(&ring->xmit_lock);
			}
		}
		mod_timer(&adapter->tx_timer, jiffies + 1);
	}
	else
		mod_timer(&adapter->tx_timer, jiffies+HZ*10);
}
...
static void igb_txbulk_timer(struct igb_adapter *adapter)
{
	...
	adapter->tx_timer.function = igb_timeout_txbulk;
...
static int igb_get_link_speed(struct net_device *netdev)
{
	struct igb_adapter *adapter = netdev_priv(netdev);
	struct e1000_hw *hw = &adapter->hw;
	unsigned short speed = 10, duplex = 0;
	switch (hw->phy.media_type) {
	case e1000_media_type_copper:
		e1000_get_speed_and_duplex_copper_generic(hw, &speed, &duplex);
		break;
	case e1000_media_type_internal_serdes:
		e1000_get_speed_and_duplex_fiber_serdes_generic(hw, &speed, &duplex);
		break;
	case e1000_media_type_unknown:
	default:
		break;
...
static void igb_set_power_up_phy(struct net_device *dev)
{
	...
	if (adapter->hw.phy.media_type == e1000_media_type_copper) {
		u16 mii_reg = 0;
		igb_read_phy_reg(&adapter->hw, PHY_CONTROL, &mii_reg);
		mii_reg &= ~(MII_CR_POWER_DOWN | MII_CR_ISOLATE);
		igb_write_phy_reg(&adapter->hw, PHY_CONTROL, mii_reg);
	}
	else{
		u32 mii_reg;
		mii_reg = E1000_READ_REG(&adapter->hw, E1000_CTRL);
		if(ilos_init_value < 0){
			if(mii_reg & E1000_CTRL_ILOS)
				ilos_init_value = 1;
			else
				ilos_init_value = 0;
		}
		if(ilos_init_value)
			mii_reg |= E1000_CTRL_ILOS;
		else
			mii_reg &= ~E1000_CTRL_ILOS;
		mii_reg |= (0x01<<23);
		E1000_WRITE_REG(&adapter->hw, E1000_CTRL, mii_reg);
	}
	netif_carrier_on(dev);
	schedule_work(&adapter->reset_task);
}
...
static void igb_get_pci_bridge_name(struct pci_dev *pdev)
{
	...
	device = pdev->device;
	if((pdev->device == 0x1533) && (use_e1000_port==0) && (nr_i210_copper==1))
	{
		strlcpy(g_iface_pci[0].name, pci_name(pdev), 13);
		g_iface_pci[0].device = device;
	}
	else
	{
	...
	if(bridge->device == 0x8616) {
		/* Detect NIC module with 8616 PCI Switch */
		...
		if(!bridge3)
			return;
		if((pdev->device == 0x1533) && (use_e1000_port==0) && (nr_i210_copper==1))
			strlcpy(g_iface_pci[0].bridge, pci_name(bridge3), 13);
		else
			strlcpy(g_iface_pci[e1000_port[cards_found]].bridge, pci_name(bridge3), 13);

	}
	else {
		if((pdev->device == 0x1533) && (use_e1000_port==0) && (nr_i210_copper==1))
			strlcpy(g_iface_pci[0].bridge, pci_name(bridge), 13);
		else
