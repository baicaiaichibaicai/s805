...
#if 1 /* rsy add for external phy */
		if( hw->mac.type == e1000_i354){
			reg = E1000_READ_REG(hw, E1000_MDICNFG);
			reg |= E1000_MDICNFG_EXT_MDIO;
			reg |= E1000_MDICNFG_COM_MDIO;
	    	E1000_WRITE_REG(hw, E1000_MDICNFG, reg);
		}
#endif		
#endif		
		reg = E1000_READ_REG(hw, E1000_MDICNFG);
...
static s32 e1000_init_phy_params_82575(struct e1000_hw *hw)
{
	...
	if (e1000_sgmii_active_82575(hw) && !e1000_sgmii_uses_mdio_82575(hw)) {
		...
		case e1000_i211:
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
			phy->ops.read_reg = e1000_read_reg_gs40g;
			phy->ops.write_reg = e1000_write_reg_gs40g;
#else			
	...
	if(phy->id == M88E1112_E_PHY_ID){
			...
			u16 data, timeout;
#if 0
			/* write reg 16_2 : Mode Select : 010 */
            ret_val = phy->ops.write_reg(hw, E1000_M88E1112_PAGE_ADDR, 2);
	        if (ret_val)
                goto out;

			ret_val = phy->ops.read_reg(hw, 16, &data);
			if (ret_val)
				goto out;
            ...
            ret_val = phy->ops.write_reg(hw, 16, data);
	        if (ret_val)
                goto out;

            ret_val = phy->ops.write_reg(hw, E1000_M88E1112_PAGE_ADDR, 0x0000);
	        if (ret_val)
                goto out;

            ret_val = phy->ops.write_reg(hw, E1000_M88E1112_PAGE_ADDR, 0x8000);
	        if (ret_val)
                goto out;

#endif

			/* write reg 16_1 : SIGDET Polarity */
            ret_val = phy->ops.write_reg(hw, E1000_M88E1112_PAGE_ADDR, 1);
	        if (ret_val)
                goto out;

            ret_val = phy->ops.write_reg(hw, 16, (0x1 << 9));
	        if (ret_val)
                goto out;

#if 0			
            ret_val = phy->ops.write_reg(hw, E1000_M88E1112_PAGE_ADDR, (0x1 << 15));
	        if (ret_val)
                goto out;
#endif			

			/* write reg 17_3 : LED Polarity */
            ret_val = phy->ops.write_reg(hw, E1000_M88E1112_PAGE_ADDR, 3);
	        if (ret_val)
                goto out;

            ret_val = phy->ops.write_reg(hw, 17, 0x55);
	        if (ret_val)
                goto out;

			/* write reg 16_3 : Modes of Operation */
            ret_val = phy->ops.write_reg(hw, 16, 0x4777);
	        if (ret_val)
                goto out;
			...
			/* reset for mode select */
#if 0			
			timeout = 1000;
            ret_val = phy->ops.write_reg(hw, 0, 0x8000);
	        if (ret_val)
                goto out;

			usleep_range(1000, 2000);

			ret_val = phy->ops.read_reg(hw, 0, &data);
			if (ret_val)
				goto out;

			while((data & 0x8000) && --timeout){
				usleep_range(1000, 2000);
				ret_val = phy->ops.read_reg(hw, 0, &data);
				if (ret_val)
					goto out;
			...
			if(!timeout)
				printk("%s : phy soft reset timeout\r\n", __func__);
#endif
...
static s32 e1000_check_for_link_media_swap(struct e1000_hw *hw)
{
	...
	printk("## rsy(fiber) : E1000_M88E1112_STATUS = 0x%x\r\n", data);
#endif
	...
	printk("## rsy(copper) : E1000_M88E1112_STATUS = 0x%x\r\n", data);
	...
	if (port == E1000_MEDIA_PORT_COPPER) {
		printk("## rsy : E1000_MEDIA_PORT_COPPER\r\n");
		/* reset page to 0 */
		ret_val = phy->ops.write_reg(hw, E1000_M88E1112_PAGE_ADDR, 0);
		if (ret_val)
			return ret_val;
		...
		printk("## rsy : E1000_MEDIA_PORT_OTHER\r\n");
		e1000_check_for_link_82575(hw);
		/* reset page to 0 */
		ret_val = phy->ops.write_reg(hw, E1000_M88E1112_PAGE_ADDR, 0);
		if (ret_val)
						...
						u16 *speed, u16 *duplex)
{
	...
	if (pcs & E1000_PCS_LSTS_LINK_OK) {
		printk("## rsy : E1000_PCS_LSTS_LINK_OK\r\n");
		...
		printk("## rsy : speed : 0x%x, duplex : 0x%x\r\n", *speed, *duplex);

	} else {
		printk("## rsy : LINK Error\r\n");
...
static s32 e1000_setup_serdes_link_82575(struct e1000_hw *hw)
{
	 ...
	 * at 1gb. Autoneg should be default set by most vers/net/ethernet/intel/igb-5.1.2/e1000_82575.c:     if (!igb_enable_mng_pass_thru(hw)) {
	 drivers/net/ethernet/intel/igb-5.1.2/e1000_82575.c:     if (!(igb_enable_mng_pass_thru(hw) || phy->ops.check_reset_block(hw)))drivers. This is the
...
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
		if (e1000_sgmii_uses_mdio_82575(hw)) {
			u32 mdicnfg = E1000_READ_REG(hw, E1000_MDICNFG);

			mdicnfg &= E1000_MDICNFG_PHY_MASK;
			hw->phy.addr = mdicnfg >> E1000_MDICNFG_PHY_SHIFT;
			printk("1000BASE_KX w external MDIO device at 0x%x\r\n", hw->phy.addr);
		} else {
			printk("1000BASE_KX\r\n");
		}
#endif
...
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
			printk("SGMII with external MDIO PHY\r\n");
#endif			
		}
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
		else {
			printk("SGMII with external I2C PHY\r\n");
		}
		break;
#endif		
...
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
			printk("SERDES with external SFP\r\n");
#endif			
			break;
		}
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
...
static void e1000_power_down_phy_copper_82575(struct e1000_hw *hw)
{
	...
	if (!(igb_enable_mng_pass_thru(hw) || phy->ops.check_reset_block(hw)))
...
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
	ret_val = e1000_read_phy_reg_mdic(hw, hw->phy.addr, offset, data);
#else
...
#if defined(CONFIG_MZEN) && defined(CONFIG_USE_MZEN_BOARD)
	ret_val = e1000_write_phy_reg_mdic(hw, hw->phy.addr, offset, data);
#else
