...
#else
#define MAX_BYPASS  	6
#define MAX_CAR_BD_SLOT	3

extern char bootserial[25];
extern int num_of_caswell_bypass_device;
extern int select_bypass_driver[];

static int start_gen2;
static int slot_order[MAX_CAR_BD_SLOT] = {0};
static char board[8];
static struct _FUTURE_BYPASS *bp_table[MAX_BYPASS] = {0,};
static struct _FUTURE_BYPASS {
	int slot_no;
	int seg_no;
} bypass_info[MAX_BYPASS];
void set_bypass_slot_order(void);
#endif /* CONFIG_FERRET_NETWORK_BYPASS */
...
int ca_bypass_modeling(void)
{
	...
	for(interface=0; interface<NET_INTERFACE_MAX; interface++) {
		...
		for(no = 0; no < bp_int[interface].seg_no; no++) {
			...
			{
				if (bp_int[interface].slot_no == 0)
					slot2++;
				else if (bp_int[interface].slot_no == 1)
					slot3++;
				else if (bp_int[interface].slot_no == 2)
					slot1++;
			}
#endif
#ifdef CONFIG_FERRET_NETWORK_BYPASS
			bypass_info[num_of_caswell_bypass_device].slot_no = bp_int[interface].slot_no;
			bypass_info[num_of_caswell_bypass_device].seg_no  = no;
			select_bypass_driver[num_of_caswell_bypass_device] = 1;
...
#ifdef CONFIG_FERRET_NETWORK_BYPASS
	start_gen2 = num_of_caswell_bypass_device;
	if (!strncmp(bootserial, "WZN201A", 7) ||
			!strncmp(bootserial, "WZN301R", 7)) {
		strcpy(board, "CAR2051");
	}
	else if(!strncmp(bootserial, "WZN601A", 7)) {
		strcpy(board, "CAR4020");
		slot_order[0] = 2;
		slot_order[1] = 0;
		slot_order[2] = 1;
	}
...
int caswell_get_bypass_device_count(void)
{
#ifdef CONFIG_FERRET
	int i,j=2,w1;
	if((!strncmp(bootserial,"WZN201A",7)) || (!strncmp(bootserial,"WZN301R",7)))
	{
		zen_bypass_info.allbp[0]=g_nic_slot[0].element[0];  //1 pair-onboard bypass
		zen_bypass_info.allbp[1]=g_nic_slot[0].element[1];
		if (num_of_caswell_bypass_device > 1)
		{
			for(i=0; i< ((num_of_caswell_bypass_device * 2) - 2); i++)
			{
				zen_bypass_info.allbp[j]=g_nic_slot[1].element[i];
				j++;
			}
		}
	}
	...
	{
		j=bp_gen3_no;
		for (w1=0; w1 < (slot1 * 2); w1++)
		{
			zen_bypass_info.allbp[j]=g_nic_slot[1].element[w1];
			j++;
		}
		
		for (w1=0; w1 < (slot2 * 2); w1++)
		{
			zen_bypass_info.allbp[j]=g_nic_slot[2].element[w1];
			j++;
		}
		
		for (w1=0; w1 < (slot3 * 2); w1++)
		{
			zen_bypass_info.allbp[j]=g_nic_slot[3].element[w1];
			j++;
		}	
	}
#endif	
	printk(KERN_INFO "found bypass devices : %d\n", num_of_caswell_bypass_device);
	return num_of_caswell_bypass_device;
}
...
void set_bypass_slot_order(void)
{
	...
	if(!strncmp(board, "CAR2051", 7)) {
		for(i=0;i<2;i++) {
			if(!bypass_info[start_gen2+i].slot_no)
				break;
			bp_table[start_gen2+index++] = &bypass_info[start_gen2+i];
		}
	}
	else if(!strncmp(board, "CAR4020", 7)) {
		for(i=0;i<3;i++) {
			int except = 0;
			for(j=0;j<MAX_BYPASS;j++) {
				/* Per slot has 2 segment */
				if(except > 1)
					break;
				if(slot_order[i] == bypass_info[start_gen2+j].slot_no) {
					bp_table[start_gen2+index++] = &bypass_info[start_gen2+j];
					except++;
				}
			}
		}
...
{
	if (port < MAX_BYPASS) {
		caswell_set_bypass(bp_table[port]->slot_no,
				bp_table[port]->seg_no, SET_BP);
		caswell_set_bpe(bp_table[port]->slot_no,
				bp_table[port]->seg_no, SET_BPE_BP);
		caswell_set_nextboot(bp_table[port]->slot_no,
				bp_table[port]->seg_no, SET_NB_NO_NORMAL);
	}
}
...
void caswell_bypass_disable(int port)
{
	if (port < MAX_BYPASS) {
		caswell_set_bypass(bp_table[port]->slot_no,
				bp_table[port]->seg_no, SET_NORMAL);
		caswell_set_bpe(bp_table[port]->slot_no,
				bp_table[port]->seg_no, SET_BPE_OPEN);
		caswell_set_nextboot(bp_table[port]->slot_no,
				bp_table[port]->seg_no, SET_NB_NORMAL);
	}
}
...
{
	if (port < MAX_BYPASS) {
		caswell_set_wd_reset(bp_table[port]->slot_no,
				bp_table[port]->seg_no, WDT_ARM_REFRESH);
	}
}
...
{
	if (port < MAX_BYPASS) {
		caswell_set_wd_timeout(bp_table[port]->slot_no,
				bp_table[port]->seg_no, timeout);
	}
}
...
void caswell_bypass_init(int port)
{
	...
	if (port < MAX_BYPASS) {
		...
		caswell_set_nextboot(bp_table[port]->slot_no,
				bp_table[port]->seg_no, SET_NB_NO_NORMAL);
		caswell_set_wd_timeout(bp_table[port]->slot_no,
				bp_table[port]->seg_no, g_wd_timeout);
		caswell_set_wd_reset(bp_table[port]->slot_no,
				bp_table[port]->seg_no, WDT_ARM_REFRESH);
	}
